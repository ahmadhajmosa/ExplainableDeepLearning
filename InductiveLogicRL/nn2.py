...
def target_vec_rule_dict(n_variable, max_uniq=2):  # create vec to symbol and symbol to vec dictinaries
    ...
    for i in range(n_variable):
        ...
        vec_to_sym[i] = []
        vec_to_sym[i].append('X' + str(i))
        vec_to_sym[i].append('~X' + str(i))
        vec_to_sym[i].append('')
        sym_to_vec['X' + str(i)] = [i,0]
        sym_to_vec['~X' + str(i)] = [i,1]
        sym_to_vec['' + str(i)] = [i,2]
