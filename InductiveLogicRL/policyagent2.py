...
class PolicyAgent:
    ...
    def _build_model(self):
        ...
        model_input = Input(shape=(self.MAX_SEQUENCE_LENGTH, self.NB_VARS), name='model_input')
        model_input2 = Input(shape=(self.MAX_SEQUENCE_LENGTH, self.NB_VARS), name='model_input2')
        ...
        with K.name_scope('Attention_layer'):
            attention_decoder = AttentionDecoder(model_hyperparameters['Bidirectional_Layer2'], self.NB_VARS,
                                                 name="output_attention_decoder", return_probabilities=False)(
                merged_vector)
            attention_prop = AttentionDecoder(model_hyperparameters['Bidirectional_Layer2'], self.NB_VARS,
        ...
        model.compile(loss='categorical_crossentropy',
                      optimizer=SGD(lr=self.learning_rate, decay=1e-6, momentum=0.9, nesterov=True, clipnorm=2.))

        losses = dict()
        losses['value_output'] = "mse"
        losses['output_attention_decoder'] = "categorical_crossentropy"
        lossWeights = dict()
        lossWeights["value_output"] = 1
        lossWeights["output_attention_decoder"] = 1
        alpha_model.compile(loss=losses, loss_weights=lossWeights,
                            optimizer=SGD(lr=self.learning_rate, decay=1e-6, momentum=0.9, nesterov=True, clipnorm=2.))
    ...
    def train(self, batch_no):
        ...
        self.write_log(train_names, [loss, gradient_norm, policy_gradient_norm, rewards[-1][0], np.sum(rewards)],
                       batch_no)
